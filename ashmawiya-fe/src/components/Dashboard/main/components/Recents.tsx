  ...
  return (
    <div id="recents" className="grid lg:grid-cols-3 text-black gap-6">
      <div className="col-span-2">
        <h1 className="text-lg text-base-content">Recent Watched Course</h1>
          ...
          {data?.map((x, i) => {
            return !x.isComplete ? (
              <Link
                ...
                className="flex items-center gap-6 p-6 bg-white hover:bg-accent transition-all delay-100 rounded-lg shadow-md cursor-pointer"
              >
      ...
      <div className="flex flex-col gap-1 p-6 bg-white rounded-lg shadow-md">
        ...
        {data?.map((x, i) => {
          return i < 3 && x.isComplete ? (
            <div
              key={x.course.id.toString()}
              className="flex items-center gap-2 p-1 hover:bg-gray-200 cursor-auto"
            >
